<div id="{{ $id }}">
  ...
  <div id="{{ $id }}contentToMove">
    <nav id="{{$id}}nav"
         class="navbar d-none d-lg-block navbar-expand-lg navbar-category-2 p-0 {{$collapsed ? "d-none" : ""}}">
      @if($menuBefore)
      ...
      <ul id="{{ $id }}navbarUl" class="navbar-nav">
        ...
        @foreach($items as $item)
          ...
          <li class="nav-item {{$firstChildrenLevel->isNotEmpty() ? 'dropdown' : ''}}">
            <a href="{{$item->url}}" class="nav-link {{$firstChildrenLevel->isNotEmpty() ? ' dropdown-toggle' : ''}}"
               data-toggle="{{$firstChildrenLevel->isNotEmpty() ? 'dropdown' : ''}}">
              ...
              <ul class="dropdown-menu">
                @foreach($firstChildrenLevel as $firstChildLevel)
                  ...
                  <li class="nav-item {{$secondChildrenLevel->isNotEmpty() ? 'dropdown' : ''}}">
                    <a class="nav-link" data-toggle="{{$secondChildrenLevel->isNotEmpty() ? 'dropdown' : ''}}"
                       href="{{$firstChildLevel->url}}">{{ $firstChildLevel->title ?? $firstChildLevel->name }}</a>
                    @if($secondChildrenLevel->isNotEmpty())
                      ...
                      <ul class="dropdown-menu">
                        @foreach($secondChildrenLevel as $secondChildLevel)
                          <li class="nav-item">
                            <a class="nav-link"
                               href="{{$secondChildLevel->url}}">{{ $secondChildLevel->title ?? $secondChildLevel->name }}</a>
                          </li>
                        @endforeach
                      </ul>
                    @endif
