  ...
  <div class="card-item {{$row}}">
    ...
    <div class="item-content {{$col2}} {{$contentPositionVertical}}">
              ...
              <div class=" {{$orderClasses["title"] ?? 'order-1'}} item-title {{$titleAlign}} ">
                  @if(isset($item->url) && !empty($item->url))
                      <a href="{{$item->url}}" target="{{$target}}">
                          @endif
                          <h3 class="title {{$titleTextWeight}} {{$titleTextTransform}} {{$titleColor}} {{$titleMarginT}} {{$titleMarginB}}">
          ...
          @if($withCreatedDate && isset($item->created_at))
              <div class=" {{$orderClasses["date"] ?? 'order-2'}} item-created-date {{$createdDateAlign}}">
                  @if(isset($item->url)&& !empty($item->url))
                      <a href="{{$item->url}}" target="{{$target}}">
                          @endif
                          <div class="created-date {{$createdDateTextWeight}} {{$createdDateColor}} {{$createdDateMarginT}} {{$createdDateMarginB}}">
          ...
          @if($withCategory && isset($item->category->id))
              <div class=" {{$orderClasses["categoryTitle"] ?? 'order-3'}} item-category {{$categoryAlign}}">
          ...
          @if($withSummary && ( isset($item->summary) || isset($item->description) || isset($item->custom_html)) )
              <div class=" {{$orderClasses["summary"] ?? 'order-4'}} item-summary {{$summaryAlign}}">
                  @if(isset($item->url) && !empty($item->url))
                      <a href="{{$item->url}}" target="{{$target}}">
                          @endif
          ...
          @if($withViewMoreButton)
              <div class="{{$orderClasses["viewMoreButton"] ?? 'order-5'}} item-view-more-button {{$buttonAlign}}">
                  @if(isset($item->url) && !empty($item->url))
                      <a href="{{$item->url}}" class="btn btn{{strpos($buttonLayout,'outline') !== false ? '-outline' : ''}}-{{$buttonColor}} {{$buttonLayout}} view-more-button {{$buttonMarginT}} {{$buttonMarginB}}" target="{{$target}}">
                          @endif
