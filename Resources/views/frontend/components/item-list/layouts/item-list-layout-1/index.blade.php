<div class="item-layout list-item-layout-1">
  <div class="card card-category bg-white border-0">
        ...
        <div class="row align-items-center">
          <div class="col-12 {{$orderClasses[0] ?? 'order-1'}}">
          ...
          <div class="col-12 {{$orderClasses[1] ?? 'order-2'}}">
            @if(isset($item->url))
              <a href="{{$item->url}}">
                @endif
            <h3 class="title">
          ...
          @if($withCreatedDate && isset($item->created_at))
            <div class="col-12 {{$orderClasses[2] ?? 'order-3'}} item-created-date">
              @if(isset($item->url))
                <a href="{{$item->url}}">
                  @endif
          ...
          @if(isset($item->category->id))
            <div class="col-12 {{$orderClasses[3] ?? 'order-4'}} item-category-title">
          ...
          @if(isset($item->summary) || isset($item->description))
            <div class="col-12 {{$orderClasses[4] ?? 'order-5'}} item-summary">
              @if(isset($item->url))
                <a href="{{$item->url}}">
                  @endif
          ...
          @if($withViewMoreButton)
            <div class="col-12 {{$orderClasses[5] ?? 'order-6'}} item-view-more-button">
              @if(isset($item->url))
