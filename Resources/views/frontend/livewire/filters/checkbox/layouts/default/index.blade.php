<div class="filter-{{$filter['type']}} filter-{{$filter['type']}}-layout-{{$filter['layout']}} filter-{{$index}}">
	...
	<div class="title">
        <a class="item mb-3" data-toggle="collapse" href="#collapse-{{$index}}" role="button" aria-expanded="{{$isExpanded ? 'true' : 'false'}}" aria-controls="collapse-{{$index}}"
	...
	<div class="content position-relative my-3">
		...
		<div class="collapse multi-collapse {{$isExpanded ? 'show' : ''}}" id="collapse-{{$index}}">
			dashdjashdj
		</div>
