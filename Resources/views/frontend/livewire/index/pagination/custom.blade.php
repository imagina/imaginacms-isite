          ...
          @if (is_array($element))
            @foreach ($element as $page => $url)
              @if ($page == $paginator->currentPage())
                <li class="page-item active" wire:key="paginator-page-{{ $page }}" aria-current="page"><span
                    class="page-link">{{ $page }}</span></li>
              @else
                <li class="page-item page-link-scroll" wire:key="paginator-page-{{ $page }}"><a
                    href="{{Request::url()}}?page={{$page}}" class="page-link"
...
<script>
  document.addEventListener("DOMContentLoaded", function () {
    ...
    if (page) {
      const targetButton = document.querySelector(`[wire\\:click\\.prevent="gotoPage(${page})"]`);
