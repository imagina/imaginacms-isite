...
@foreach($items as $key => $item)
  <div class="{{$layoutClass[$key%count($layoutClass)]}} {{$itemMainClass}}" onclick="checkModal_{{$itemModal['idModal']}}({{$item->id}})" >
    ...
    $component = $__env->getContainer()->make($itemComponentNamespace, array_merge($itemComponentAttributes, ["item" => $item,"itemListLayout"=>$itemListLayout,"editLink"=>$editLink,"tooltipEditLink"=>$tooltipEditLink, "currentRoute" => $currentRoute, "printedItemCount" => $key]));
