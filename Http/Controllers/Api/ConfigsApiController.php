...
class ConfigsApiController extends BaseApiController
{
  ...
  public function index(Request $request)
  {
    try {
      ...
      $configName = $params->filter->configName ?? $params->filter->configNameByModule ?? null;//Get config name filter
      $byModule = isset($params->filter->configNameByModule);//Get config name by module filter
      //Get config
    ...
    try {
      ...
      if (isset($params->filter->asSelect) && $params->filter->asSelect) {
        ...
        foreach ($modulesInfoAsSelect as $item) {
          if (isset($item->entities)) unset($item->entities);
        }
